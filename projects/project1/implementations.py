...
def gradient_descent(y, tx, w, max_iters, gamma, gradient=gradient, loss=MSE_loss, lambda_=0):
    """Perform gradient descent optimization for the MSE loss function.
    ...
    Args:
        ...
        w: numpy array of shape=(D, ).
            The initial vector of model parameters.
        max_iters: int.
            The number of iterations to run the gradient descent.
        gamma: float.
            The learning rate.
        gradient: function, optional.
            The gradient computation function. Default is `gradient`.
        loss: function, optional.
            The loss function to minimize. Default is `MSE_loss`.
        lambda_: float, optional.
            The regularization parameter (default is 0, meaning no regularization).

    Returns:
        numpy array of shape=(D, ).
            The optimized model parameters after `max_iters` iterations.
    """
    for i in range(max_iters):
        w = w - gamma * (gradient(y, tx, w) + 2 * lambda_ * w)
    return w


def stochastic_gradient_descent(
    y, tx, w, max_iters, gamma, batch_size, gradient=gradient, loss=MSE_loss, lambda_=0
):
    """Perform stochastic gradient descent (SGD) optimization for the MSE loss function.
    ...
    Args:
        ...
        w: numpy array of shape=(D, ).
            The initial vector of model parameters.
        max_iters: int.
            The number of iterations to run the stochastic gradient descent.
        gamma: float.
            The learning rate.
        batch_size: int.
            The size of each mini-batch for the stochastic gradient computation.
        gradient: function, optional.
            The gradient computation function. Default is `gradient`.
        loss: function, optional.
            The loss function to minimize. Default is `MSE_loss`.
...
def adam(
    y, tx, w, max_iters, gamma, beta1=0.9, beta2=0.999, gradient=gradient, loss=MSE_loss
):
    """Perform optimization using the Adam algorithm for the MSE loss function.
    ...
    Args:
        ...
        w: numpy array of shape=(D, ).
            The initial vector of model parameters.
        max_iters: int.
            The number of iterations to run the Adam optimization.
        gamma: float.
            The learning rate.
...
def ridge_regression(y, tx, lambda_):
    ...
    w = np.linalg.solve(
        tx.T @ tx + 2 * y.shape[0] * lambda_ * np.eye(tx.shape[1]), tx.T @ y
    )
...
def clamp_between_0_and_1(t):
    """Clamp the input values to the range [1e-12, 1 - 1e-12].
    This function ensures that all elements of the input array `t` are bounded
    between a very small positive value and slightly less than 1.

    Args:
        t: numpy array or scalar.
            The input values to be clamped.

    Returns:
        numpy array or scalar.
            The clamped values, with all elements in the range [1e-12, 1 - 1e-12].
    """
...
def cross_validation_one_step(
    y, x, k_indices, k, lambda_, function_name, initial_w, max_iters, gamma
):
    ...
    y_tr, x_tr = (
        y[k_indices[train_indices].flatten()],
        x[k_indices[train_indices].flatten()],
    )
    ...
    match function_name:
        case "mean sqrt":
            ...
            return (
                np.sqrt(2 * loss_tr),
                np.sqrt(2 * loss_te),
                f1_and_accuracy(x_tr, x_te, y_tr, y_te, w),
            )
        case "mean sqrt sgd":
        ...
        case "logistic regression adam":
            w, loss_tr = logistic_regression_adam(
                y_tr, x_tr, initial_w, max_iters, gamma
            )
            loss_te = loss_logistic(y_te, x_te, w)
            return (
                np.sqrt(2 * loss_tr),
        ...
        case "mean sqrt adam":
            w, loss_tr = mean_squared_error_adam(
                y_tr, x_tr, initial_w, max_iters, gamma
            )
            loss_te = MSE_loss(y_te, x_te, w)
            return (
        ...
        case _:
            raise ValueError(
                "Function not recognized, choose between: mean sqrt, mean sqrt sgd, "
...
def replace_column_with(
    array1, array2, columns, values_to_replace, value_to_replace_with
):
